...
* '_closeQuotemarkCountChange': Some cards are missing opening or closing quotemarks, leading to a Verifier difference because of a mismatched count. The value of this override gets added to the 'close quotemark'-count so this can be corrected. See also the '_openQuotemarkCountChange'-field
* '_listEntryLengthChange': Some list fields are lists of dictionaries, and the number of keys in these dictionaries, their length, is compared between English and non-English cards, and if they differ, a difference is flagged. If that difference is correct, this correction can be provided. It's a dictionary, with the field name as the key, and a list as value, with the first list item the field index and the second list item the change to the expected length
...
* '_openQuotemarkCountChange': Some cards are missing opening or closing quotemarks, leading to a Verifier difference because of a mismatched count. The value of this override gets added to the 'open quotemark'-count so this can be corrected. See also the '_closeQuotemarkCountChange'-field
